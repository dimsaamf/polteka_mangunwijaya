@extends('rolesuperadmin.layoutsuperadmin.editpengguna')
@section('content')
...
<div class="bg-abu-polteka w-full min-h-[500px] px-9 md:rounded-xl rounded-[30px] md:mt-0 md:ml-0 md:mr-0 mt-6 ml-8 mr-8">
    ...
    <section class="w-full mt-2 mb-5 h-14 border-b border-slate-300">
        ...
        <div class="flex md:hidden my-4 w-1/2 justify-start text-sm">
            <div class="text-hitam-polteka">Edit Pengguna</div>
        </div> 
        <div class="hidden md:flex my-4 w-1/2 justify-start text-xs sm:text-md md:text-[13px] lg:text-lg">
            ...
            <div class="ml-2  text-hitam-polteka">Edit Pengguna</div>
        </div> 
    ...
    <section class="text-hitam-polteka my-8  bg-white rounded-lg p-6">
        <h2 class="text-xl font-medium">Edit Pengguna</h2>
        <form action="{{ route('updatepenggunasuperadmin', $user->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <label class="block mt-4">
                <span class="text-sm font-medium">Username*</span>
                <input type="text" name="name" value="{{ $user->name }}" class="mt-2 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="Username Pengguna" />
                @error('name')
            ...
            <label class="block mt-4">
                ...
                <input type="email" name="email" value="{{ $user->email }}" class="mt-2 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="Email Pengguna" />
                @error('email')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </label>
            <label class="block mt-4">
                ...
                <input type="password" name="password" class="mt-2 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="Password Baru" />
                @error('password')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </label>
            <label class="block mt-4">
                ...
                <div class="group">
                    ...
                    <select name="role" class="mt-2 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-sky-500 focus:border-sky-500 sm:text-sm">
                        ...
                        <option value="wakildirektur" {{ $user->role == 'wakildirektur' ? 'selected' : '' }}>Wakil Direktur</option>
                        <option value="superadmin" {{ $user->role == 'superadmin' ? 'selected' : '' }}>Superadmin</option>
                        <option value="adminprodfarmasi" {{ $user->role == 'adminprodfarmasi' ? 'selected' : '' }}>Admin Prodi Farmasi</option>
                        <option value="adminprodkimia" {{ $user->role == 'adminprodkimia' ? 'selected' : '' }}>Admin Prodi Kimia</option>
                        <option value="adminprodankes" {{ $user->role == 'adminprodankes' ? 'selected' : '' }}>Admin Prodi Analisis Kesehatan</option>
                        <option value="adminlabprodfarmasi" {{ $user->role == 'adminlabprodfarmasi' ? 'selected' : '' }}>Admin Lab Prodi Farmasi</option>
                        <option value="adminlabprodkimia" {{ $user->role == 'adminlabprodkimia' ? 'selected' : '' }}>Admin Lab Prodi Kimia</option>
                        <option value="adminlabprodankes" {{ $user->role == 'adminlabprodankes' ? 'selected' : '' }}>Admin Lab Prodi Analisis Kesehatan</option>
                        <option value="koorlabprodfarmasi" {{ $user->role == 'koorlabprodfarmasi' ? 'selected' : '' }}>Koor Lab Prodi Farmasi</option>
                        <option value="koorlabprodkimia" {{ $user->role == 'koorlabprodkimia' ? 'selected' : '' }}>Koor Lab Prodi Kimia</option>
                        <option value="koorlabprodankes" {{ $user->role == 'koorlabprodankes' ? 'selected' : '' }}>Koor Lab Prodi Analisis Kesehatan</option>
                    </select>
                    @error('role')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
