@extends('rolekoorlabankes.layoutkoorlab.labmikro.databarang')
@section('content')
...
<div class="bg-abu-polteka w-full min-h-[500px] px-9 md:rounded-xl rounded-[30px] md:mt-0 md:ml-0 md:mr-0 mt-6 ml-8 mr-8">
    ...
    <section class="text-hitam-polteka my-8  bg-white rounded-lg p-6">
        ...
        <form action="{{ route('updatebarangkoorlabmikro', $labmikro->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <label class="block mt-4">
            <span class="text-sm font-medium">Nama*</span>
            <input type="text" name="nama_barang" value="{{ $labmikro->nama_barang }}" class="mt-2 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="Nama Barang" />
            @error('nama_barang')
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 md:gap-7 gap-4 mt-4">
            <label class="block md:col-span-2">
                ...
                <input type="number" name="jumlah" value="{{ $labmikro->jumlah }}" readonly class="mt-2 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="Jumlah Barang" />
                @error('jumlah')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </label>
                    ...
                    <div class="group">
                        <div class="inline-flex w-full justify-start mt-2 rounded-md bg-white px-3 py-2.5 text-sm text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                            <select  name="satuan" id="satuan" class="form-control @error('satuan') is-invalid @enderror w-full" required>
                                ...
                                <option value="ml" {{ $labmikro->satuan == 'ml' ? 'selected' : '' }}>ml</option>
                                <option value="gr" {{ $labmikro->satuan == 'gr' ? 'selected' : '' }}>gr</option>
                                <option value="pcs" {{ $labmikro->satuan == 'pcs' ? 'selected' : '' }}>pcs</option>
                                <option value="lembar" {{ $labmikro->satuan == 'lembar' ? 'selected' : '' }}>lembar</option>
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <label class="block md:col-span-1">
                ...
                <input name="jumlah_min" value="{{ $labmikro->jumlah_min }}" class="mt-2 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="Jumlah Minimal" />
                @error('jumlah_min')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 md:gap-7 gap-4 mt-4">
            <label class="block mt-4 md:col-span-2" id="tanggal_service_field" @if($labmikro->tanggal_service) style="display:block;" @else style="display:none;" @endif>
                <span class="text-sm font-medium">Tanggal Service</span>
                <input type="date" name="tanggal_service" value="{{ $labmikro->tanggal_service }}" class="mt-2 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" min="{{ now()->format('Y-m-d') }}" />
            </label>
            <label class="block mt-4  md:col-span-1" id="periode_field" @if($labmikro->tanggal_service) style="display:block;" @else style="display:none;" @endif>
                <span class="text-sm font-medium">Periode Service</span>
                <div class="flex mt-2">
                    <input type="number" name="periode" value="{{ $labmikro->periode }}" class="px-3 py-[9px] bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="Periode Service" min="0" max="12" />
        ...
        <label class="block mt-4">
            ...
            <input type="name" name="harga" value="{{ $labmikro->harga }}" class="mt-2 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="Harga Barang" />
            @error('harga')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </label>
        <label class="block mt-4">
            ...
            <input type="name" name="keterangan" value="{{ $labmikro->keterangan }}" class="mt-2 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="Keterangan" />
            @error('keterangan')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </label>
