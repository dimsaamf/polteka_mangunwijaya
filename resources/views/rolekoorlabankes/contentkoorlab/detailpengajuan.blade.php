@extends('rolekoorlabankes.layoutkoorlab.detailpengajuan')
@section('content')
            ...
            <h2 class="text-xl font-semibold mb-5">Detail Pengajuan Barang</h2>
                    ...
                    <div>
                        ...
                        @if (pathinfo($pengajuanBarang->file, PATHINFO_EXTENSION) == 'pdf')
                            <embed src="{{ route('preview.surat.koorlabankes', ['id' => $pengajuanBarang->kode_pengajuan]) }}" type="application/pdf" width="100%" height="600px">
                        @else
                            <img src="{{ route('preview.surat.koorlabankes', ['id' => $pengajuanBarang->kode_pengajuan]) }}" alt="Gambar Pengajuan" class="w-full h-auto">
                        @endif
                    </div>
        </div>
    </section>
