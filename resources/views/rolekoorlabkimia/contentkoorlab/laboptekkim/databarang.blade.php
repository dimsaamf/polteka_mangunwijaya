@extends('rolekoorlabkimia.layoutkoorlab.laboptekkim.databarang')
@section('content')
    ...
    <section class="text-hitam-polteka">
                ...
                <div class="overflow-hidden">
                    <div class="flex">
                        <div class="flex w-1/2 justify-start mb-3">
                            <a href="{{ route('tambahbarangkoorlaboptekkim') }}" type="button" class="w-[130px] mb-3 rounded-md px-3 py-2 text-sm bg-merah200-polteka text-putih-polteka shadow-sm">
                                Tambah Barang
                            </a>
                        </div>
                        <div class="flex w-1/2 justify-end mt-2">
                            <div class ="bg-merah180-polteka w-2/3 h-10 flex items-center rounded-l-full rounded-r-full">
                                <form action="{{ route('databarangkoorlaboptekkim') }}" method="GET" class="relative flex w-full">
                    ...
                    <tbody>
                        ...
                        <tr class="text-center bg-putih-polteka border-y-8 border-abu-polteka">
                            ...
                            {{-- <td class="px-6 py-2 whitespace-nowrap text-center flex justify-center items-center rounded-l-xl">
                                ...
                                <td class="px-6 py-2 whitespace-nowrap rounded-l-xl">{{ ($laboptekkim->currentPage() - 1) * $laboptekkim->perPage() + $loop->index + 1 }}</td>
                            <td class="px-6 py-2 whitespace-nowrap text-center flex justify-center items-center rounded-l-xl">
                            ...
                            <td class="px-6 py-2 whitespace-nowrap">
                                    <div class="flex justify-center">
                                        @if($data->gambar)
                                            <a href="{{ route('get.gambar.invlaboptekkim.koor', ['id' => $data->id]) }}" target="_blank">
                            ...
                            <td class="px-6 py-2 whitespace-nowrap rounded-r-xl">
                                <a href="{{ route('ubahbarangkoorlaboptekkim', $data->id) }}" >
                            ...
                            <td class="px-6 py-2 whitespace-nowrap rounded-r-xl">
                                    ...
                                    <form action="{{ route('hapusbarangoptekkim', $data->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
        ...
        <div class="flex flex-col my-12 py-4 items-center space-y-5 overflow-x-auto">
            <ul class="inline-flex mx-autospace-x-2">
                @if ($laboptekkim->onFirstPage())
                    <li>
                        <span class="px-4 py-2 text-gray-400 text-sm">Sebelumnya</span>
                    </li>
                @else
                    <li>
                        <a href="{{ $laboptekkim->previousPageUrl() }}" class="px-4 py-2 text-hitam-polteka hover:font-bold text-sm">Sebelumnya</a>
                    </li>
                @endif
        
                @foreach ($laboptekkim->getUrlRange($laboptekkim->currentPage() - 2, $laboptekkim->currentPage() + 2) as $page => $url)
                    @if ($page == $laboptekkim->currentPage())
                        <li>
                ...
                @if ($laboptekkim->hasMorePages())
                    <li>
                        <a href="{{ $laboptekkim->nextPageUrl() }}" class="px-4 py-2 text-hitam-polteka hover:font-bold hover:text-hitam-polteka text-sm">Selanjutnya</a>
                    </li>
                @else
                    <li>
        ...
        $('.btndelete').click(function (e){
            ...
            swal({
                ...
                if (willDelete) {
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: '/koorlabkimia/laboptekkim/databarang/' + deleteid,
                        data: data,
