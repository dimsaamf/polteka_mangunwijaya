    ...
    <section class="text-hitam-polteka">
            ...
            <div class="-m-1.5 overflow-x-auto">
                ...
                <div class="flex">
                    ...
                    <div class="flex w-1/2 justify-start mb-3 items-center">
                        <form action="{{ route('riwayatbarangkeluarkoorlaboptekkim') }}" method="GET" class="items-center flex gap-3 my-auto">
                        ...
                        <form action="{{ route('riwayatbarangkeluarkoorlaboptekkim') }}" method="GET">
                    ...
                    <div class="flex w-1/2 justify-end mt-5 mb-3">
                                    ...
                                    <form action="{{ route('riwayatbarangkeluarkoorlaboptekkim') }}" method="GET" class="relative flex w-full">
        ...
        <div class="flex flex-col my-12 py-4 items-center space-y-5 overflow-x-auto">
            <ul class="inline-flex mx-autospace-x-2">
                @if ($BarangKeluarOptekkim->onFirstPage())
                    <li>
                        <span class="px-4 py-2 text-gray-400 text-sm">Sebelumnya</span>
                    </li>
                @else
                    <li>
                        <a href="{{ $BarangKeluarOptekkim->previousPageUrl() }}" class="px-4 py-2 text-hitam-polteka hover:font-bold text-sm">Sebelumnya</a>
                    </li>
                @endif
        
                @foreach ($BarangKeluarOptekkim->getUrlRange($BarangKeluarOptekkim->currentPage() - 2, $BarangKeluarOptekkim->currentPage() + 2) as $page => $url)
                    @if ($page == $BarangKeluarOptekkim->currentPage())
                        <li>
                ...
                @if ($BarangKeluarOptekkim->hasMorePages())
                    <li>
                        <a href="{{ $BarangKeluarOptekkim->nextPageUrl() }}" class="px-4 py-2 text-hitam-polteka hover:font-bold hover:text-hitam-polteka text-sm">Selanjutnya</a>
                    </li>
                @else
                    <li>
