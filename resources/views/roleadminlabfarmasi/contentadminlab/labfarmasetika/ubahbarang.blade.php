    ...
    <section class="text-hitam-polteka my-8  bg-white rounded-lg p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 md:gap-7 gap-4 mt-4">
                    ...
                    <div class="group">
                        <div class="inline-flex w-full justify-start mt-2 rounded-md bg-white px-3 py-2.5 text-sm text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                            <select  name="satuan" id="satuan" class="form-control @error('satuan') is-invalid @enderror w-full" required>
                                ...
                                <option value="ml" {{ $labfarmasetika->satuan == 'ml' ? 'selected' : '' }}>ml</option>
                                <option value="gr" {{ $labfarmasetika->satuan == 'gr' ? 'selected' : '' }}>gr</option>
                                <option value="pcs" {{ $labfarmasetika->satuan == 'pcs' ? 'selected' : '' }}>pcs</option>
                                <option value="lembar" {{ $labfarmasetika->satuan == 'lembar' ? 'selected' : '' }}>lembar</option>
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <label class="block md:col-span-1">
                ...
                <input name="jumlah_min" value="{{ $labfarmasetika->jumlah_min }}" class="mt-2 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="Jumlah Minimal" />
                @error('jumlah_min')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 md:gap-7 gap-4 mt-4">
            <label class="block mt-4 md:col-span-2" id="tanggal_service_field" @if($labfarmasetika->tanggal_service) style="display:block;" @else style="display:none;" @endif>
                ...
                <input type="date" name="tanggal_service" value="{{ $labfarmasetika->tanggal_service }}" readonly class="mt-2 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" min="{{ now()->format('Y-m-d') }}" />
            </label>
            <label class="block mt-4  md:col-span-1" id="periode_field" @if($labfarmasetika->tanggal_service) style="display:block;" @else style="display:none;" @endif>
                ...
                <div class="flex mt-2">
                    <input type="number" name="periode" value="{{ $labfarmasetika->periode }}" readonly class="px-3 py-[9px] bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="Periode Service" min="0" max="12" />
