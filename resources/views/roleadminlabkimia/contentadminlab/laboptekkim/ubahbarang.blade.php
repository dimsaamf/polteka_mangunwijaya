@extends('roleadminlabkimia.layoutadminlab.laboptekkim.ubahbarang')
@section('content')
...
<div class="bg-abu-polteka w-full min-h-[500px] px-9 md:rounded-xl rounded-[30px] md:mt-0 md:ml-0 md:mr-0 mt-6 ml-8 mr-8">
    ...
    <section class="text-hitam-polteka my-8  bg-white rounded-lg p-6">
        ...
        <form action="{{ route('updatebarangadminlaboptekkim', $laboptekkim->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 md:gap-7 gap-4 mt-4">
            <label class="block mt-4 md:col-span-2" id="tanggal_service_field" @if($laboptekkim->tanggal_service) style="display:block;" @else style="display:none;" @endif>
                ...
                <input type="date" name="tanggal_service" value="{{ $laboptekkim->tanggal_service }}" readonly class="mt-2 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" min="{{ now()->format('Y-m-d') }}" />
            </label>
            <label class="block mt-4  md:col-span-1" id="periode_field" @if($laboptekkim->tanggal_service) style="display:block;" @else style="display:none;" @endif>
                ...
                <div class="flex mt-2">
                    <input type="number" name="periode" value="{{ $laboptekkim->periode }}" readonly class="px-3 py-[9px] bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="Periode Service" min="0" max="12" />
