@extends('roleadminlabkimia.layoutadminlab.labkimiaanalisa.barangmasuk')
@section('content')
    ...
    <section class="text-hitam-polteka">
            ...
            <div class="-m-1.5 overflow-x-auto">
                ...
                <div class="flex">
                    ...
                    <div class="flex w-1/2 justify-start mb-3 items-center">
                        <form action="{{ route('riwayatbarangmasukadminlabkimiaanalisa') }}" method="GET" class="items-center flex gap-3 my-auto">
                        ...
                        <form action="{{ route('riwayatbarangmasukadminlabkimiaanalisa') }}" method="GET">
                    ...
                    <div class="flex w-1/2 justify-end mt-5 mb-3">
                                    ...
                                    <form action="{{ route('riwayatbarangmasukadminlabkimiaanalisa') }}" method="GET" class="relative flex w-full">
                    ...
                    <tbody>
                        @if ($BarangMasukKimiaAnalisa->isEmpty())
                            <tr>
                                <td colspan="8" class="px-6 py-4 text-center">Tidak ada data yang tersedia.</td>
                            </tr>
                        @else
                        @foreach($BarangMasukKimiaAnalisa as $item)
        ...
        <div id="default-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-auto fixed inset-0">
            ...
            <div class="fixed inset-0 overflow-y-auto flex items-center justify-center z-50">
                ...
                <div class="relative bg-white rounded-lg shadow w-[300px] sm:w-[450px] md:w-[500px]">
                    ...
                    <!-- Modal body -->
                    <div class="px-8">
                    <form action="{{ route('barangmasukadminlabkimiaanalisa.store') }}" method="POST" enctype="multipart/form-data">  
                    @csrf
        ...
        <div class="flex flex-col my-12 py-4 items-center space-y-5 overflow-x-auto">
            <ul class="inline-flex mx-autospace-x-2">
                @if ($BarangMasukKimiaAnalisa->onFirstPage())
                    <li>
                        <span class="px-4 py-2 text-gray-400 text-sm">Sebelumnya</span>
                    </li>
                @else
                    <li>
                        <a href="{{ $BarangMasukKimiaAnalisa->previousPageUrl() }}" class="px-4 py-2 text-hitam-polteka hover:font-bold text-sm">Sebelumnya</a>
                    </li>
                @endif
        
                @foreach ($BarangMasukKimiaAnalisa->getUrlRange($BarangMasukKimiaAnalisa->currentPage() - 2, $BarangMasukKimiaAnalisa->currentPage() + 2) as $page => $url)
                    @if ($page == $BarangMasukKimiaAnalisa->currentPage())
                        <li>
                ...
                @if ($BarangMasukKimiaAnalisa->hasMorePages())
                    <li>
                        <a href="{{ $BarangMasukKimiaAnalisa->nextPageUrl() }}" class="px-4 py-2 text-hitam-polteka hover:font-bold hover:text-hitam-polteka text-sm">Selanjutnya</a>
                    </li>
                @else
                    <li>
