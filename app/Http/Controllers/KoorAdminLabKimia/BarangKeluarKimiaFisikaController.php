...
class BarangKeluarKimiaFisikaController extends Controller
{
    public function tabel(Request $request){
        ...
        // return view('rolekoorlabkimia.contentkoorlab.labkimiafisika.barangkeluar', compact('data'));
        if(session('is_logged_in')) {
            if(Auth::user()->role == 'koorlabprodkimia'){
                return view('rolekoorlabkimia.contentkoorlab.labkimiafisika.barangkeluar', compact('data'));
            } else{
                return view('roleadminlabkimia.contentadminlab.labkimiafisika.barangkeluar', compact('data'));
            }
        }
    }

    public function index(Request $request){
        // $BarangKeluarKimiaFisika = BarangKeluarKimiaFisika::paginate(10);
        ...
        $queryBuilder = BarangKeluarKimiaFisika::with('InventarisLabKimiaFisika')
        ...
        if(session('is_logged_in')) {
            if(Auth::user()->role == 'koorlabprodkimia'){
                ...
                return view('roleadminlabkimia.contentadminlab.labkimiafisika.riwayatkeluar', compact('BarangKeluarKimiaFisika','data'));
            }
        }
    }

    public function create(){
        // return view('rolekoorlabkimia.contentkoorlab.labkimiafisika.barangkeluar');
        if(session('is_logged_in')) {
            if(Auth::user()->role == 'koorlabprodkimia'){
                return view('rolekoorlabkimia.contentkoorlab.labkimiafisika.barangkeluar');
            } else{
                return view('roleadminlabkimia.contentadminlab.labkimiafisika.barangkeluar');
            }
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        if( $jumlah_awal < $jumlah_keluar_baru ){
            ...
            $BarangKeluarKimiaFisika = new BarangKeluarKimiaFisika();
            $BarangKeluarKimiaFisika->jumlah_keluar = $jumlah_keluar_baru;
            $BarangKeluarKimiaFisika->tanggal_keluar = $request->tanggal_keluar;
            $BarangKeluarKimiaFisika->keterangan_keluar = $request->keterangan_keluar;
            $BarangKeluarKimiaFisika->id_barang = $id_barang;
            $BarangKeluarKimiaFisika->save();

            alert()->success('Berhasil','Stok Barang Berhasil Dikurangi.');
            // return redirect()->route('barangkeluarkoorlabkimiafisika');
            if(session('is_logged_in')) {
                if(Auth::user()->role == 'koorlabprodkimia'){
                    return redirect()->route('barangkeluarkoorlabkimiafisika');
                } else{
                    return redirect()->route('barangkeluaradminlabkimiafisika');
                }
            }
        }
    }
